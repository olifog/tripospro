...
async def process_year(db, course_obj, year, year_data):
    course_year_obj = await db.courseyear.find_first(
        ...
        include={"Question": True, "TriposPartYear": {"include": {"triposPart": True}}},
    )
    ...
    tasks = [
        process_question(
            db, course_obj, course_year_obj, question_number, question_data
        )
...
async def process_question(
    ...
    if question_obj is None:
        print(
            "Question not found", course_obj.code, course_year_obj.year, question_number
        )
        return None

    question_url = question_data["question_url"]
    question_text = await get_question_text(
        "https://www.cl.cam.ac.uk/teaching/exams/pastpapers/" + question_url
    )
    ...
    if course_year_obj.TriposPartYear is not None:
        metadata["triposPart"] = course_year_obj.TriposPartYear.triposPart.name
...
async def main():
    ...
    for course, course_data in questions.items():
        ...
        course_obj = await db.course.find_first(where={"code": course})
        ...
        all_course_vectors = [
            vector for vectors in all_course_vectors for vector in vectors
        ]

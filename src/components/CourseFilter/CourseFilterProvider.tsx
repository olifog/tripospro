...
const HideCurrentYearCheckbox = () => {
  const { hideCurrentYear, setHideCurrentYear } =
    useContext(CourseFilterContext);
  return (
    <CheckboxWithLabel
      id="hideCurrentYear"
      checked={hideCurrentYear}
      onCheckedChange={setHideCurrentYear}
...
const CurrentYearCheckbox = () => {
  ...
  return (
    <CheckboxWithLabel
      ...
      label="Only show currently taught courses"
    />
  );
};
...
const OnlyExaminedCheckbox = () => {
  ...
  return (
    <CheckboxWithLabel
      id="onlyExamined"
      checked={onlyExamined}
      onCheckedChange={setOnlyExamined}
      label="Only show examined courses"
    />
  );
};
...
const YearCutoffInput = () => {
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      >
        Year cutoff:{" "}
        <span className="text-slate-900 dark:text-slate-100">{yearCutoff}</span>
      </Label>
      <Slider
        id="yearCutoff"
        value={yearCutoff ? [parseInt(yearCutoff)] : [1993]}
        onValueChange={([value]) => setYearCutoff(value.toString())}
        min={1993}
...
export const CourseFilterProvider = ({
  ...
  const [onlyExamined, setOnlyExamined] = useLocalStorage(
    "onlyExamined",
    true,
    {
      initializeWithValue: false,
  ...
  const [hideCurrentYear, setHideCurrentYear] = useLocalStorage(
    "hideCurrentYear",
    true,
    {
      initializeWithValue: false,
